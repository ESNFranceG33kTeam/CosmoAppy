...
func NewVolunteer(vlt *Volunteer) {
	stmt, _ := TheDb().Prepare("INSERT INTO volunteers (firstname, lastname, email, discord, phone, university, postal_address, actif, bureau, hr_status, started_date) VALUES (?,?,?,?,?,?,?,?,?,?,?);")
	_, err := stmt.Exec(vlt.Firstname, vlt.Lastname, vlt.Email, vlt.Discord, vlt.Phone, vlt.University, vlt.PostalAddress, vlt.Actif, vlt.Bureau, vlt.HrStatus, vlt.StartedDate)
	if err != nil {
...
func FindVolunteerById(id int) *Volunteer {
	...
	err := row.Scan(&vlt.Id, &vlt.Firstname, &vlt.Lastname, &vlt.Email, &vlt.Discord, &vlt.Phone, &vlt.University, &vlt.PostalAddress, &vlt.Actif, &vlt.Bureau, &vlt.HrStatus, &vlt.StartedDate)
...
func AllVolunteers() *Volunteers {
	...
	for rows.Next() {
		...
		err := rows.Scan(&vlt.Id, &vlt.Firstname, &vlt.Lastname, &vlt.Email, &vlt.Discord, &vlt.Phone, &vlt.University, &vlt.PostalAddress, &vlt.Actif, &vlt.Bureau, &vlt.HrStatus, &vlt.StartedDate)
...
func UpdateVolunteer(vlt *Volunteer) {
	stmt, err := TheDb().Prepare("UPDATE volunteers SET firstname=?, lastname=?, email=?, discord=?, phone=?, university=?, postal_address=?, actif=?, bureau=?, hr_status=?, started_date=? WHERE id=?;")

	if err != nil {
		TheLogger().LogError("volunteer", "problem with the db.", err)
	}

	_, err = stmt.Exec(vlt.Firstname, vlt.Lastname, vlt.Email, vlt.Discord, vlt.Phone, vlt.University, vlt.PostalAddress, vlt.Actif, vlt.Bureau, vlt.HrStatus, vlt.StartedDate, vlt.Id)
